// DeepSeek Web 对话导入 —— Client 半部（动态 Cordis 插件浏览器 UI）
// 用法：把本文件内容作为 cordis_define 的 code.client 传入（与 host.js 配对）。
return {
  apply(ctx) {
    ...
    function App(props) {
      ...
      React.useEffect(function () {
        (async function () {
          const s = await host.call('tokenStatus');
          setStatus(s);
          const w = await host.call('listWorkspaces');
      ...
      async function saveToken() {
        setError(''); setNotice('');
        const r = await host.call('saveToken', { token: token });
        if (r && r.ok) { setNotice(r.normalized ? 'Token 已保存（已自动提取 value 字段）' : 'Token 已保存'); setStatus(await host.call('tokenStatus')); }
        else setError((r && r.error) || '保存失败');
      }
      async function clearToken() {
        await host.call('clearToken');
        setToken(''); setNotice('Token 已清除'); setStatus(await host.call('tokenStatus'));
      }
      async function list() {
        setLoading(true); setError(''); setSessions(null);
        if (token.trim()) {
          const s = await host.call('saveToken', { token: token });
          if (s && s.ok) { setStatus(await host.call('tokenStatus')); }
          else { setLoading(false); setError((s && s.error) || 'Token 保存失败'); return; }
        }
        const r = await host.call('listSessions');
        setLoading(false);
      ...
      async function doImport(s) {
        ...
        const r = await host.call('importToSession', { sessionId: s.id, title: s.title, workspaceId: wid });
        setResults(function (p) { const n = {}; for (const k in p) n[k] = p[k]; n[s.id] = r; return n; });
      }
      async function runProbe() {
        setProbeOut('');
        const r = await host.call('probe', { url: probeUrl, method: 'GET' });
    ...
    slots.inject('settings.section', () => slots.register(
      { name: 'settings.section', id: 'deepseek-web-import', order: 30, label: 'DeepSeek 对话导入' },
      (props) => React.createElement(App, { close: props && props.close })
    ));
  }
};
