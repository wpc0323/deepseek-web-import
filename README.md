...
**根因**：DSH 的 JSONL 会话日志要求事件 `seq` 从 0 连续递增；若上一次运行以单独的一次 append 写入了 `session/end-seed`（会话收尾时），而下次运行恢复时的游标没有把这条算进去，新写入的第一批事件就会**复用已提交的 seq**，导致提交区截断在重复处、后面的事件全部读不出来。**这是 DSH 自身的恢复游标问题，与本插件无关**（插件导入的会话把 end-seed 放在同一批写入中）。
